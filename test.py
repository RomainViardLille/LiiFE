...
def show_both_bundles(bundles, colors=None, show=True, fname=None):
    ...
    if fname is not None:
        sleep(1)
        window.record(scene, n_frames=1, out_path=fname, size=(300,300))
...
""" show_both_bundles([cb_subj1, cb_subj2],
                  ...
                  show=False,
                  fname=os.path.join(extracted_path,"QC","bundlesS",f"before_registration_{file_name}.jpg")) """

show_both_bundles([cb_subj1, cb_subj2_aligned],
                  colors=[window.colors.orange, window.colors.red],
                  show=False,
                  fname=os.path.join(extracted_path,"QC","bundlesS",f"after_registration_{file_name}.jpg"))
